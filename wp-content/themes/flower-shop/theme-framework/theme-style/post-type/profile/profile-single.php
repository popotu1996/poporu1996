...
if (
	$cmsmasters_option['flower-shop' . '_profile_post_cat'] || 
	$cmsmasters_option['flower-shop' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			flower_shop_profile_title_nolink(get_the_ID(), 'h2', $cmsmasters_profile_subtitle, 'h4');
		echo '</header>';
	}
		...
		echo '<div class="profile_sidebar">';
			...
			if ($profile_details == 'true') {
				...
				echo '<div class="profile_details entry-meta">';
					...
					flower_shop_get_profile_likes('post');
					
					flower_shop_get_profile_comments('post');
					
					flower_shop_get_profile_features('details', $cmsmasters_profile_features, false, 'h3', true);
					
					flower_shop_get_profile_category(get_the_ID(), 'pl-categs', 'post');
					
				echo '</div>';
			}
			
			
			flower_shop_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h5', true);
			
			flower_shop_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h5', true);
			
			flower_shop_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h5', true);
			
			
			flower_shop_profile_open_social_icons($cmsmasters_profile_social, get_the_ID(), esc_html__('Socials', 'flower-shop'), 'h5');
...
if ($cmsmasters_option['flower-shop' . '_profile_post_nav_box']) {
	$order_cat = (isset($cmsmasters_option['flower-shop' . '_profile_post_nav_order_cat']) ? $cmsmasters_option['flower-shop' . '_profile_post_nav_order_cat'] : false);
