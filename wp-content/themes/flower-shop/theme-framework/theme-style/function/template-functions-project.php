...
function flower_shop_project_heading($cmsmasters_id, $tag = 'h1', $link_redirect = false, $link_url = false, $link_target = false, $show = true) { 
...
function flower_shop_project_title_nolink($cmsmasters_id, $tag = 'h1', $show = true) { 
...
function flower_shop_project_exc_cont($show = true) {
	$out = cmsmasters_divpdel('<div class="cmsmasters_project_content entry-content">' . "\n" . 
		wpautop(flower_shop_excerpt(20, false)) . 
	'</div>' . "\n");
...
function flower_shop_project_check_exc_cont() {
	$exc = flower_shop_project_exc_cont(false);
...
function flower_shop_get_project_category($cmsmasters_id, $taxonomy, $template_type = 'page', $show = true) {
	$out = '';
	...
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		if ($template_type == 'page') {
			...
			if ($cmsmasters_option['flower-shop' . '_portfolio_project_cat']) {
				$out .= '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Categories', 'flower-shop') . ':' . '</div>' . 
...
function flower_shop_get_project_likes($template_type = 'page', $show = true) {
	$out = '';
	...
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['flower-shop' . '_portfolio_project_like']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Likes', 'flower-shop') . ':' . '</div>' . 
...
function flower_shop_get_project_comments($template_type = 'page', $show = true) {
	$out = '';
	
	
	if (comments_open()) {
		if ($template_type == 'page') {
			$out = flower_shop_get_comments('cmsmasters_project_comments');
		} elseif ($template_type == 'post') {
			$cmsmasters_option = flower_shop_get_global_options();
			
			if ($cmsmasters_option['flower-shop' . '_portfolio_project_comment'] && comments_open()) {
				$out = '<div class="project_details_item">' . 
					'<div class="project_details_item_title">' . esc_html__('Comments', 'flower-shop') . ':' . '</div>' . 
					'<div class="project_details_item_desc details_item_desc_comments">' . 
						flower_shop_get_comments('cmsmasters_project_comments') . 
					'</div>' . 
				'</div>';
			}
		}
...
function flower_shop_get_project_date($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		...
		if ($cmsmasters_option['flower-shop' . '_portfolio_project_date']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Date', 'flower-shop') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function flower_shop_get_project_author($template_type = 'page', $show = true) {
	if ($template_type == 'page') {
		$out = '<span class="cmsmasters_project_author">' . 
			esc_html__('By', 'flower-shop') . ' ' . 
			'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'flower-shop') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
		'</span>';
	} elseif ($template_type == 'post') {
		$cmsmasters_option = flower_shop_get_global_options();
		...
		if ($cmsmasters_option['flower-shop' . '_portfolio_project_author']) {
			$out .= '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Author', 'flower-shop') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
					'<a href="' . esc_url(get_author_posts_url(get_the_author_meta('ID'))) . '" title="' . esc_attr__('Projects by', 'flower-shop') . ' ' . esc_attr(get_the_author_meta('display_name')) . '" class="vcard author" rel="author"><span class="fn">' . esc_html(get_the_author_meta('display_name')) . '</span></a>' . 
				'</div>' . 
			'</div>';
...
function flower_shop_get_project_tags($cmsmasters_id, $taxonomy, $show = true) {
	if (get_the_terms($cmsmasters_id, $taxonomy)) {
		$cmsmasters_option = flower_shop_get_global_options();
		$out = '';
		
		if ($cmsmasters_option['flower-shop' . '_portfolio_project_tag']) {
			$out = '<div class="project_details_item">' . 
				'<div class="project_details_item_title">' . esc_html__('Tags', 'flower-shop') . ':' . '</div>' . 
				'<div class="project_details_item_desc">' . 
...
function flower_shop_get_project_features($features_position = 'features', $features = '', $features_title = false, $tag = 'h2', $show = true) {
	if (
		(
			!empty($features[0][0]) || 
			!empty($features[0][1])
...
function flower_shop_project_link($link_text, $link_url, $link_target, $show = true) {
	$cmsmasters_option = flower_shop_get_global_options();
	$out = '';
	...
	if ( 
		...
		$out = '<div class="project_details_item">' . 
			'<div class="project_details_item_title">' . esc_html__('Project Link', 'flower-shop') . ':' . '</div>' . 
			'<div class="project_details_item_desc">' . 
...
function cmsmasters_project_more($cmsmasters_id, $show = true) {
	...
	if ($cmsmasters_project_read_more == '') {
		$cmsmasters_project_read_more = esc_html__('Read More...', 'flower-shop');
	}
